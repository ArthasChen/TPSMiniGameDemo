...
public class TPSCamera : MonoBehaviour
{
    ...
    private float maxAngleDelta;//每帧角度旋转的最大值，防止摄像机翻转
    ...
    [Header("上下偏移")]
    public float localOffsetVerticle = 0;
    [Header("左右偏移")]
    public float localOffsetHorizonal = 0;
    public bool isAiming = false;//是否正在瞄准
    private float playerHeight = 0;//玩家高度

    private bool isControlView=true;//是否控制视角
    //鼠标移动量
    public float axisX;
    public float axisY;
    private void Awake()
    {
        ...
        transform.position = player.Find("CameraInitPos").position;
        transform.rotation = player.Find("CameraInitPos").rotation;
        ...
        //设置每帧垂直旋转的最大角度，防止摄像机上下翻转
        maxAngleDelta = 170-Mathf.Abs(maxAngle-minAngle);
    ...
    private void Update()
    {
        ...
        {
            //if (isAiming)
            //    DrawCrossHair._instance.Hide();
            //else
            //    DrawCrossHair._instance.Show();
            isAiming = !isAiming;
        }
        ...
        //获取鼠标移动量
        axisX = Input.GetAxis("Mouse X") * rotateSpeed * Time.deltaTime;
        axisY = Input.GetAxis("Mouse Y") * rotateSpeed * Time.deltaTime;

        if(isControlView)
            SetPosAndRot(axisX,axisY);//设置视角旋转后的位置和朝向
    ...
    /// </summary>
    public void SetPosAndRot(float axisX,float axisY)
    {
        //限制每帧旋转的角度
        axisY = (axisY>0?axisY:-axisY) > maxAngleDelta ? (axisY>0?1:-1)*maxAngleDelta : axisY;
    ...
    public void SetLocalOffset()
    {
        ...
        for(localOffsetCollider=0; !CheckView(checkPos);localOffsetCollider+=0.2f)//让localOffset递增直至没有遮挡
        {
            ...
            if (localOffsetCollider > 10f)
                break;
        }
        localOffset += localOffsetCollider;//加上这个试探出的偏移量

        Vector3 offsetPos = new Vector3(0, localOffsetVerticle, localOffset);//这是调整后相机应该移向的位置
        offsetPos.x = isAiming ? localOffsetHorizonal : 0;
        //使相机平滑移动到这个位置
        //float tempY=Mathf.Lerp(cam.)
    ...
    private bool CheckView(Vector3 checkPos)
    {
        ...
        RaycastHit hit;
        //射线终点为玩家物体的中间位置
        Vector3 endPos = player.position + player.up * playerHeight * 0.5f;
