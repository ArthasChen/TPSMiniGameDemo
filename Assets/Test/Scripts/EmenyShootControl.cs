...
public class EmenyShootControl : MonoBehaviour
{
    ...
    private void Attack()
    {
        // 开火
        Shoot();
        DetermineDistanceWhenFire();
    }

    protected 怪物状态 状态 = 怪物状态.没有目标;
    ...
    void DetermineDistanceWhenFire()
    {
        float distance = Vector3.Distance(this.transform.position, targetTrans.transform.position);
        // 先判定听觉
        if (distance > 听觉范围)
        {
            状态 = 怪物状态.没有目标;
    ...
    private void Shoot()
    {
        ...
        //// 计算血量
        //RaycastHit hit;
        //if (Physics.Raycast(this.transform.position, targetTrans.transform.forward, out hit, range, ~(1 << 9)))
        //{
